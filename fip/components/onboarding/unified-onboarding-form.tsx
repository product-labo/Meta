...
const createFormSchema = (selectedChain?: ChainType) => {
  return z.object({
    ...
    utility: z.string().min(1, 'Please select a utility type'),
  ...
  }) satisfies z.ZodType<UnifiedOnboardingFormData>
}

export interface UnifiedOnboardingFormProps {
  onComplete: (projectId: string, walletId: string) => void
  onError?: (error: string) => void
  className?: string
}

export interface UnifiedOnboardingFormData {
  // Project fields
  companyName: string
  abi?: string
  utility: string
...
export function UnifiedOnboardingForm({ 
  ...
  const form = useForm<UnifiedOnboardingFormData>({
  ...
  const onSubmit = async (data: UnifiedOnboardingFormData) => {
    ...
    try {
      ...
      const projectPayload = {
        ...
        chain: data.chain,
        contractAddress: data.address, // Map address to contractAddress
        abi: data.abi,
        utility: data.utility,
        status: 'active'
      }

      const projectResult = await api.projects.create(projectPayload, token)
      const projectId = projectResult.data?.id || projectResult.id

      if (!projectId) {
        throw new Error('Failed to create project - no project ID returned')
      }

      // Step 2: Create wallet and start indexing
      const walletPayload = {
      ...
      const walletResult = await api.wallets.create(projectId, walletPayload, token)
      const walletId = walletResult.data?.id || walletResult.id
    ...
    <div className={className}>
      <div className="space-y-6">
        <div className="space-y-2">
          <h2 className="text-2xl font-semibold tracking-tight">Create Project & Start Indexing</h2>
          <p className="text-muted-foreground">
            Fill out the details below to create your project and start indexing blockchain data
          </p>
        </div>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Contract ABI (Optional)</FormLabel>
                  <FormControl>
                    <Textarea
                      placeholder="Paste your contract ABI here for enhanced transaction decoding..."
                      className="min-h-[100px] resize-none"
                      {...field}
                    />
                  </FormControl>
                  <FormDescription>
                    Providing your contract ABI enables better transaction decoding and analytics
                  </FormDescription>
                  <FormMessage />
                </FormItem>
              )}
            ...
            >
              {isSubmitting ? (
                <>
                  ...
                  Creating Project & Starting Indexing...
                </>
              ) : (
                'Create Project & Start Indexing'
              )}
            </Button>
          </form>
        </Form>
