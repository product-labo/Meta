...
`;
  ...
  /**
   * Generate real-time alerts based on analysis data
   */
  async generateRealTimeAlerts(analysisResults, previousResults = null, userId = 'anonymous') {
    this.initialize();
    ...
    try {
      const prompt = `
Analyze this smart contract data and generate real-time alerts for potential issues:
...
ALERT CRITERIA:
- Security: Unusual transaction patterns, potential exploits, suspicious addresses
- Performance: High gas usage, failed transactions, slow execution
- Liquidity: Low TVL, high slippage, liquidity drain
- Anomaly: Sudden volume spikes, whale activity, unusual user behavior
- Growth: Declining users, reduced activity, market position changes

Important rules:
- Return ONLY valid JSON, no markdown or explanations
- Focus on actionable alerts that require immediate attention
