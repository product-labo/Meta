...
export async function performContinuousContractSync(analysisId, config, userId) {
  console.log(`🔄 Starting continuous contract sync for user ${userId} (interaction-based)`);
  ...
  const runSyncCycle = async () => {
    try {
      ...
      console.log(`🔄 Running sync cycle ${syncCycle} for user ${userId} (interaction-based)`);
      ...
      if (lastProcessedBlock === null) {
        ...
        const baseBlockRange = config.analysisParams.blockRange || 1000;
